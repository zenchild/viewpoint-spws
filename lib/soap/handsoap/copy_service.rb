...
module Viewpoint
  module Sharepoint
    class CopyService < Handsoap::Service
      ...
      # GetItem[http://msdn.microsoft.com/en-us/library/copy.copy.getitem.aspx]
      def get_item(url)
        soap_action = SOAP_ACTION_PREFIX + 'GetItem'
        response = invoke('spsoap:GetItem', :soap_action => soap_action) do |root|
          build!(root) do
            url!(url)
          end
        end
      ...
      def build!(node, opts = {}, &block)
        CopyBuilder.new(node, opts, &block)
      end

      def parse!(response, opts = {})
        CopyParser.new(response).parse(opts)
      end
    
    end # CopyService class
